...
//zinx0.9连接管理模块(增加链接客户端的个数的限制，达到后，会拒绝链接)
//创建链接管理模块(定义，属性，方法)，集成到zinx框架，定义创建链接之前和销毁链接之后所需要处理的业务，提供给用户能够注册hook函数。
//connManager属性：已经创建的conn的map，针对map(增删改查)做一个保护锁，方法：添加链接，删除链接，查找链接，总链接个数获取，清理全部连接
//集成：添加到server模块，添加连接属性，判断连接数目是否超出最大值，server推出前，清除所有链接
//zinx的hook函数:给server添加属性和方法(创建连接自动调用hook，销毁连接前调用hook)，设置OnConnStart和OnConnStop方法
//zinx1.0连接属性
//给connection模块添加可以配置属性的功能(连接属性的集合(name之类的)，锁)(方法：增删查)
...
func DoConnectionBegin(conn ziface.IConnection)  {
	fmt.Println("----->begin hook is call")
	if err := conn.Send(202,[]byte("do connection begin"));err != nil {
		fmt.Println(err)
	}
	//给当前连接添加属性
	fmt.Println("set conn Name....")
	conn.SetProper("name","linyu")
	conn.SetProper("home","www.baidu.com")

}

//连接之后的钩子函数
func DoConnectionlost(conn ziface.IConnection)  {
	fmt.Println("----->last hook is call")
	fmt.Println("conn ID=",conn.GetConnID(),"is login")
	if home,err := conn.GetProper("home"); err == nil {
